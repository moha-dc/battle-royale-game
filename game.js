...
let state = {
  ...
  player: null, remotePlayers: {},
  trees: [], rocks: [], chests: [],
  walls: [], bullets: [], drops: [],
  particles: [], damageNumbers: [],
  storm: null, camera: { x: 0, y: 0 },
  alive: 0, kills: 0
};
...
async function init() {
  ...
  state.kills = 0; state.walls = []; state.bullets = [];
  state.drops = []; state.particles = []; state.damageNumbers = [];
  state.remotePlayers = {};

  const data = await api({ action: 'join' });
  myId = data.id;
  generateMap(data.mapSeed);
  state.player = makeFighter(data.player.x, data.player.y, true);
  state.player.color = data.player.color;
  ...
  window.addEventListener('beforeunload', () => {
    navigator.sendBeacon(API, JSON.stringify({ action: 'leave', id: myId }));
  });
}
...
async function syncGame() {
  if (!state.running || !myId || !state.player) return;
  const p = state.player;
  let data;
  try {
    data = await api({
      action: 'sync', id: myId,
      player: { x:p.x, y:p.y, angle:p.angle, hp:p.hp, shield:p.shield, alive:p.alive, slots:p.slots, slotIndex:p.slotIndex, wood:p.wood, brick:p.brick }
    });
  } catch { return; }

  // Update storm
  if (data.storm) {
    state.storm = data.storm;
    const outside = dist(p.x, p.y, data.storm.cx, data.storm.cy) > data.storm.r;
    document.getElementById('storm-warn').classList.toggle('hidden', !outside);
    const info = document.getElementById('zone-info');
    info.textContent = outside ? 'TEMPÊTE !' : 'SÉCURISÉE';
    info.style.color  = outside ? '#ff5252' : '#fff';
  }

  // Update remote players
  state.remotePlayers = {};
  for (const pid in data.players) {
    if (pid !== myId) state.remotePlayers[pid] = data.players[pid];
  }

  // Apply server-authoritative HP (storm damage, hits by others)
  if (data.me) {
    const prevHp = p.hp;
    p.hp     = data.me.hp;
    p.shield = data.me.shield;
    p.kills  = data.me.kills;
    if (data.me.hp < prevHp) {
      p.flash = 8;
      const fl = document.getElementById('flash');
      fl.style.opacity = '1'; setTimeout(() => fl.style.opacity = '0', 80);
    }
    if (!data.me.alive && p.alive) { p.alive = false; endGame(false); return; }
  }

  // Chests opened by others
  if (data.openedChests) {
    for (const idx of data.openedChests) {
      if (state.chests[idx]) state.chests[idx].opened = true;
    }
  }
  ...
  updateHUD();
  if (data.alive === 1 && p.alive && state.running) endGame(true);
}

// ============================================================
// Report a hit to server
// ============================================================
async function reportHit(targetId, damage) {
  const data = await api({ action: 'hit', attackerId: myId, targetId, damage });
  if (data.alive === 1 && state.player?.alive && state.running) endGame(true);
}
...
function generateMap(seed) {
  ...
  for (let i = 0; i < TREE_COUNT; i++)
    state.trees.push({ x:200+rng.float()*(WORLD-400), y:200+rng.float()*(WORLD-400), hp:40, r:22, type:rng.float()<.5?0:1 });
...
function updatePlayer() {
  ...
  if (dx||dy) { const m=Math.hypot(dx,dy); p.vx+=(dx/m)*p.speed*sprint*.4; p.vy+=(dy/m)*p.speed*sprint*.4; }
...
function fireWeapon(f) {
  const slot=f.slots[f.slotIndex]; if (!slot) return;
  const w=WEAPONS[slot.type]; if (!w) return;
  f.cd=w.cd;
  if (w.kind==='melee') { meleeHit(f,w); spawnSwing(f); }
  else {
    const pellets=w.pellets||1;
    for (let i=0;i<pellets;i++) {
      const ang=f.angle+(Math.random()-.5)*w.spread*2;
      state.bullets.push({ x:f.x+Math.cos(f.angle)*22, y:f.y+Math.sin(f.angle)*22, vx:Math.cos(ang)*w.speed, vy:Math.sin(ang)*w.speed, dmg:w.dmg, range:w.range, traveled:0, owner:f, color:w.color });
    }
    for (let i=0;i<6;i++)
      state.particles.push({ x:f.x+Math.cos(f.angle)*24, y:f.y+Math.sin(f.angle)*24, vx:Math.cos(f.angle+(Math.random()-.5)*.6)*(2+Math.random()*3), vy:Math.sin(f.angle+(Math.random()-.5)*.6)*(2+Math.random()*3), life:10, color:'#ffea00', r:3 });
  }
}

function meleeHit(f, w) {
  const tx=f.x+Math.cos(f.angle)*w.range*.6, ty=f.y+Math.sin(f.angle)*w.range*.6;
  for (const t of state.trees) {
    if (t.hp<=0) continue;
    if (dist(t.x,t.y,tx,ty)<w.range*.6) {
  ...
  for (const r of state.rocks) {
    if (r.hp<=0) continue;
    if (dist(r.x,r.y,tx,ty)<w.range*.6) {
  ...
  if (f.isPlayer) {
    for (const id in state.remotePlayers) {
      ...
      if (dist(rp.x,rp.y,tx,ty)<w.range*.6) { reportHit(id, w.dmg); spawnParticles(rp.x,rp.y,'#ff5252',6); return; }
    }
  }
  for (const wl of state.walls) {
    if (wl.hp<=0) continue;
    if (dist(wl.x,wl.y,tx,ty)<w.range*.6) { wl.hp-=w.dmg; spawnParticles(wl.x,wl.y,'#c98a4a',4); return; }
  }
}

function spawnSwing(f) {
  for (let i=0;i<5;i++) { const a=f.angle+(Math.random()-.5)*.8; state.particles.push({x:f.x+Math.cos(a)*30,y:f.y+Math.sin(a)*30,vx:Math.cos(a)*2,vy:Math.sin(a)*2,life:8,color:'#fff',r:2}); }
}

function tryMove(f) {
  const nx=f.x+f.vx, ny=f.y+f.vy; let bx=false, by=false;
  ...
  for (const t of state.trees) {
    if (t.hp<=0) continue;
    if (dist(nx,f.y,t.x,t.y)<f.r+t.r*.6) bx=true;
    if (dist(f.x,ny,t.x,t.y)<f.r+t.r*.6) by=true;
  }
  for (const r of state.rocks) {
    if (r.hp<=0) continue;
    if (dist(nx,f.y,r.x,r.y)<f.r+r.r*.7) bx=true;
    if (dist(f.x,ny,r.x,r.y)<f.r+r.r*.7) by=true;
  }
  if (!bx) f.x=nx; if (!by) f.y=ny;
  f.x=clamp(f.x,f.r,WORLD-f.r); f.y=clamp(f.y,f.r,WORLD-f.r);
}

function rectCircle(rx,ry,rw,rh,cx,cy,cr) { return dist(cx,cy,clamp(cx,rx,rx+rw),clamp(cy,ry,ry+rh))<cr; }
...
function updateBullets() {
  for (let i=state.bullets.length-1;i>=0;i--) {
    ...
    let hit=false;
    for (const w of state.walls) { if (w.hp<=0) continue; if (Math.abs(b.x-w.x)<30&&Math.abs(b.y-w.y)<30) { w.hp-=b.dmg; spawnParticles(b.x,b.y,'#c98a4a',4); hit=true; break; } }
    if (hit) { state.bullets.splice(i,1); continue; }
    for (const t of state.trees) { if (t.hp<=0) continue; if (dist(b.x,b.y,t.x,t.y)<t.r*.6) { t.hp-=b.dmg*.4; spawnParticles(b.x,b.y,'#5c3a1e',3); hit=true; break; } }
    if (hit) { state.bullets.splice(i,1); continue; }
    for (const r of state.rocks) { if (r.hp<=0) continue; if (dist(b.x,b.y,r.x,r.y)<r.r*.8) { r.hp-=b.dmg*.5; spawnParticles(b.x,b.y,'#aaa',3); hit=true; break; } }
    if (hit) { state.bullets.splice(i,1); continue; }
    for (const id in state.remotePlayers) {
      const rp=state.remotePlayers[id]; if (!rp.alive) continue;
      if (dist(b.x,b.y,rp.x,rp.y)<18) { reportHit(id,b.dmg); spawnParticles(b.x,b.y,'#ff5252',6); hit=true; break; }
    }
...
function spawnParticles(x,y,color,n) {
  for (let i=0;i<n;i++) state.particles.push({x,y,vx:(Math.random()-.5)*5,vy:(Math.random()-.5)*5,life:20+Math.random()*15,color,r:2+Math.random()*2});
}
...
function updateCamera() {
  state.camera.x+=(state.player.x-state.camera.x)*.12;
  state.camera.y+=(state.player.y-state.camera.y)*.12;
}
...
function buildWall() {
  const p=state.player; if (!p?.alive||(p.wood||0)<10) return;
  ...
  state.walls.push({x:wx,y:wy,hp:100}); p.wood-=10;
  spawnParticles(wx,wy,'#c98a4a',8); updateHUD();
}

function tryPickup() {
  const p=state.player; if (!p?.alive) return;
  for (let i=0;i<state.chests.length;i++) {
    const c=state.chests[i]; if (c.opened) continue;
    if (dist(p.x,p.y,c.x,c.y)<50) {
      c.opened=true;
      api({action:'openChest',id:myId,index:i});
      openChest(c); return;
    }
  }
...
function openChest(c) {
  const loot=[]; const n=1+Math.floor(Math.random()*2);
  for (let i=0;i<n;i++) loot.push({kind:'weapon',type:GUN_TYPES[Math.floor(Math.random()*GUN_TYPES.length)]});
  if (Math.random()<.5) loot.push({kind:'shield'});
  if (Math.random()<.5) loot.push({kind:'heal'});
...
function pickupDrop(d) {
  ...
  spawnParticles(p.x,p.y,'#5dffac',10); updateHUD();
}
...
function drawGround() {
  const sx=Math.max(0,Math.floor((state.camera.x-W/2)/TILE)*TILE), sy=Math.max(0,Math.floor((state.camera.y-H/2)/TILE)*TILE);
  const ex=Math.min(WORLD,sx+W+TILE*2), ey=Math.min(WORLD,sy+H+TILE*2);
  for (let x=sx;x<ex;x+=TILE) for (let y=sy;y<ey;y+=TILE) { ctx.fillStyle=((x/TILE+y/TILE)%2===0)?'#5fb04a':'#56a542'; ctx.fillRect(x,y,TILE,TILE); }
...
function drawDrops() {
  for (const d of state.drops) {
    ...
    if (d.kind==='weapon') { const w=WEAPONS[d.type]; ctx.fillStyle=w.color; ctx.shadowColor=w.color; ctx.shadowBlur=12; ctx.fillRect(-12,-4,24,8); ctx.fillStyle='#222'; ctx.fillRect(-4,-7,6,4); }
    else if (d.kind==='heal') { ctx.shadowColor='#5dffac'; ctx.shadowBlur=12; ctx.fillStyle='#fff'; ctx.fillRect(-10,-10,20,20); ctx.fillStyle='#ff3a3a'; ctx.fillRect(-8,-3,16,6); ctx.fillRect(-3,-8,6,16); }
    else if (d.kind==='shield') { ctx.shadowColor='#3da9fc'; ctx.shadowBlur=12; ctx.fillStyle='#3da9fc'; ctx.beginPath(); ctx.moveTo(0,-12); ctx.lineTo(10,-6); ctx.lineTo(8,8); ctx.lineTo(0,12); ctx.lineTo(-8,8); ctx.lineTo(-10,-6); ctx.closePath(); ctx.fill(); }
    ctx.restore(); ctx.shadowBlur=0;
    if (dist(d.x,d.y,state.player.x,state.player.y)<40) { ctx.fillStyle='#ffea00'; ctx.font='bold 12px JetBrains Mono'; ctx.textAlign='center'; ctx.fillText('[E]',d.x,d.y-22); }
  }
}
...
function drawBullets() {
  for (const b of state.bullets) {
    ...
    ctx.beginPath(); ctx.moveTo(b.x,b.y); ctx.lineTo(b.x-b.vx*1.5,b.y-b.vy*1.5); ctx.stroke(); ctx.globalAlpha=1;
  }
}

function drawFighters() {
  for (const id in state.remotePlayers) { const rp=state.remotePlayers[id]; if (rp.alive) drawFighter(rp); }
...
function drawFighter(f) {
  ...
  ctx.fillStyle=f.flash>0?'#fff':f.color; ctx.beginPath(); ctx.arc(f.x,f.y,f.r,0,Math.PI*2); ctx.fill();
  ctx.strokeStyle=f.isPlayer?'#000':'rgba(0,0,0,.6)'; ctx.lineWidth=3; ctx.beginPath(); ctx.arc(f.x,f.y,f.r,0,Math.PI*2); ctx.stroke();
  ctx.save(); ctx.translate(f.x,f.y); ctx.rotate(f.angle);
  const slot=f.slots&&f.slots[f.slotIndex];
  if (slot) { const w=WEAPONS[slot.type]; if (w.kind==='melee') { ctx.fillStyle='#bbb'; ctx.fillRect(14,-3,18,6); ctx.fillStyle='#5c3a1e'; ctx.fillRect(8,-2,10,4); } else { ctx.fillStyle=w.color; ctx.fillRect(8,-4,22,8); ctx.fillStyle='#222'; ctx.fillRect(28,-2,4,4); } }
  ctx.fillStyle='#1a1a2e'; ctx.fillRect(2,-8,12,5); ctx.fillStyle='#3da9fc'; ctx.fillRect(3,-7,10,2);
  ctx.restore();
...
function drawSmallBar(x,y,ratio,color) {
  const w=36,h=4; ctx.fillStyle='rgba(0,0,0,.6)'; ctx.fillRect(x-w/2,y,w,h);
  ctx.fillStyle=color; ctx.fillRect(x-w/2,y,w*Math.max(0,ratio),h);
}

function drawParticles() {
  for (const p of state.particles) { ctx.globalAlpha=Math.max(0,p.life/30); ctx.fillStyle=p.color; ctx.beginPath(); ctx.arc(p.x,p.y,p.r,0,Math.PI*2); ctx.fill(); }
  ctx.globalAlpha=1;
}
...
function drawStorm() {
  const s=state.storm;
  ctx.save();
  ctx.fillStyle='rgba(180,70,220,.25)'; ctx.beginPath(); ctx.rect(0,0,WORLD,WORLD); ctx.arc(s.cx,s.cy,s.r,0,Math.PI*2,true); ctx.fill('evenodd');
  ...
  if (s.r>s.targetR+1) { ctx.setLineDash([10,10]); ctx.strokeStyle='rgba(255,255,255,.5)'; ctx.lineWidth=2; ctx.beginPath(); ctx.arc(s.cx,s.cy,s.targetR,0,Math.PI*2); ctx.stroke(); ctx.setLineDash([]); }
  ctx.restore();
}

function onScreen(x,y,pad) { return x>state.camera.x-W/2-pad&&x<state.camera.x+W/2+pad&&y>state.camera.y-H/2-pad&&y<state.camera.y+H/2+pad; }

function drawMinimap() {
  const size=160; mctx.fillStyle='#3a6a2a'; mctx.fillRect(0,0,size,size);
  const sc=size/WORLD, s=state.storm;
  mctx.fillStyle='rgba(180,70,220,.4)'; mctx.fillRect(0,0,size,size);
  mctx.globalCompositeOperation='destination-out'; mctx.beginPath(); mctx.arc(s.cx*sc,s.cy*sc,s.r*sc,0,Math.PI*2); mctx.fill();
  mctx.globalCompositeOperation='source-over';
  mctx.strokeStyle='#c44fff'; mctx.lineWidth=1.5; mctx.beginPath(); mctx.arc(s.cx*sc,s.cy*sc,s.r*sc,0,Math.PI*2); mctx.stroke();
  for (const id in state.remotePlayers) { const rp=state.remotePlayers[id]; if (!rp.alive) continue; mctx.fillStyle=rp.color||'#ff5252'; mctx.fillRect(rp.x*sc-1.5,rp.y*sc-1.5,3,3); }
  if (state.player?.alive) { mctx.fillStyle='#ffea00'; mctx.beginPath(); mctx.arc(state.player.x*sc,state.player.y*sc,3,0,Math.PI*2); mctx.fill(); mctx.strokeStyle='#000'; mctx.lineWidth=1; mctx.stroke(); }
...
function updateHUD() {
  ...
  for (let i=0;i<3;i++) {
    const slot=document.getElementById('slot-'+(i+1)), s=p.slots[i];
...
function endGame(victory) {
  if (!state.running) return;
  state.running=false;
  api({action:'leave',id:myId});
  const title=document.getElementById('end-title'), stats=document.getElementById('end-stats');
  if (victory) { title.textContent='VICTOIRE !'; title.className='title victory'; stats.innerHTML=`Dernier survivant !<br><span>${state.kills}</span> éliminations`; }
  else { title.textContent='ÉLIMINÉ'; title.className='title defeat'; stats.innerHTML=`<span>${state.kills}</span> éliminations`; }
